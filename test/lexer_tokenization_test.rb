...
module Zaid
  class LexerTokenizationTest < Minitest::Test
    include Keywords
    ...
    def test_identifier
      assert_equal [[:IDENTIFIER, 'عدد']], Lexer.new.tokenize('عدد')
    end

    def test_float
      assert_equal [[:FLOAT, 3.14]], Lexer.new.tokenize('٣.١٤')
      assert_equal [[:FLOAT, 3.14]], Lexer.new.tokenize('3.14')
    end

    def test_number
      assert_equal [[:NUMBER, 314]], Lexer.new.tokenize('٣١٤')
      assert_equal [[:NUMBER, 314]], Lexer.new.tokenize('314')
    end

    def test_mixed_digits
      assert_equal [[:NUMBER, 314]], Lexer.new.tokenize('3١٤')
    end

    def test_string
      assert_equal [[:STRING, 'زيد']], Lexer.new.tokenize('"زيد"')
    end
    ...
    def test_operator
      ...
      assert_equal [[:NUMBER, 5], ['>=', '>='], [:NUMBER, 3]], Lexer.new.tokenize('5 >=3')
      assert_equal [[:NUMBER, 5], ['>=', '>='], [:NUMBER, 3]], Lexer.new.tokenize('5>= 3')
      assert_equal [[:NUMBER, 5], ['>=', '>='], [:NUMBER, 3]], Lexer.new.tokenize('5>=3')
    end
    ...
    def test_arabic_operators
      assert_equal [[:NUMBER, 5], [:EQUALS, EQUALS], [:NUMBER, 3]],
                   Lexer.new.tokenize('٥ يساوي ٣', run_compression: false)

      assert_equal [[:NUMBER, 5], [:NOT, NOT], [:EQUALS, EQUALS], [:NUMBER, 3]],
                   Lexer.new.tokenize('٥ لا يساوي ٣', run_compression: false)

      assert_equal [[:NUMBER, 5], [:GREATER, GREATER], [:THAN, THAN], [:NUMBER, 3]],
                   Lexer.new.tokenize('٥ أكبر من ٣', run_compression: false)

      assert_equal [[:NUMBER, 5], [:LESS, LESS], [:THAN, THAN], [:NUMBER, 3]],
                   Lexer.new.tokenize('٥ أصغر من ٣', run_compression: false)

      assert_equal [[:NUMBER, 5], [:GREATER, GREATER], [:THAN, THAN], [:OR, OR], [:EQUALS, EQUALS], [:NUMBER, 3]],
                   Lexer.new.tokenize('٥ أكبر من أو يساوي ٣', run_compression: false)

      assert_equal [[:NUMBER, 5], [:LESS, LESS], [:THAN, THAN], [:OR, OR], [:EQUALS, EQUALS], [:NUMBER, 3]],
                   Lexer.new.tokenize('٥ أصغر من أو يساوي ٣', run_compression: false)
    end
    ...
    def test_if_statement
      ...
      assert_equal tokens, Lexer.new.tokenize(code, run_compression: false)
    end
